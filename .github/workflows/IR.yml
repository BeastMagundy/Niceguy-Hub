...
local Window = Library.CreateLib("NIGGER HUB", "DarkTheme")
...
MainSection:NewButton("Aimlock", "Hold Q, or hold RightClick to lock onto your enemies!", function()
    pcall(function()
        local PlaceName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId)
        if string.find(PlaceName.Name, "Island") and game.PlaceId ~= 1320186298 then
        wait (10)
        mouse = game.Players.LocalPlayer:GetMouse()
        mouse.KeyDown:connect(function(k)
        if k == "n" then
        pcall(function()
        local abc = getsenv(game:GetService("Players").LocalPlayer.PlayerScripts["Client_Code"]["Client_Manager"])
        local abcd = getsenv(game:GetService("Players").LocalPlayer.PlayerScripts["Bus_Handles"])
        abc._G.doingEmote = false
        abcd._G.playerChuting = false
        abcd._G.hasLanded = true
        end)
        end
        end)
        ...
        local shotgunaimspeed = 1.1
        local ARaimspeed      = 5
        local FlintSpeed      = 3
        local swappedaimspeed = 3.5
        _G.SmgKey             = "2"
        
        local movethemouse    = mousemoverel or Input.MoveMouse
        local leftclickme     = nil 
        ...
        local CircleFill      = false
        local CircleThick     = 1.5
        local CircleColor     = Color3.fromRGB(99, 64, 255)
        local CircleTrans     = 1
        ...
        local function RandomNumber()
            _G.r = rng:NextNumber(-0.4, 1)
            _G.g = rng:NextNumber(0.5, 0)
            _G.b = rng:NextNumber(-0.4, 1)
            return Vector3.new(_G.r, _G.g, _G.b)
        end
        spawn(function()
        while wait (0.1) do
        if _G.randomlock == true and _G.MB2Held == true then
        RandomNumber()
        end
        end
        end)
        ...
        circle.Transparency = CircleTrans
        circle.Color = CircleColor
        circle.Thickness = CircleThick
        circle.Radius = 100
        circle.Filled = CircleFill
        circle.NumSides = 100
        circle.Radius = _G.Fov
        circle.Position =
        Vector2.new(
        math.floor(currentCamera.ViewportSize.X / 1.899),
        ...
        coroutine.wrap(
            function()
                while true do
                    pcall(function()
                    local teamStorage = game:GetService("ReplicatedStorage").Team_Data
                    if teamStorage then
                        team = {}
        ...
        spawn(function()
        if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChildOfClass("Tool") and string.find(game.Players.LocalPlayer.Character:FindFirstChildOfClass("Tool").Name, "Shotgun") then
        _G.Distance = 150
        else
        _G.Distance = 800
        end
            ...
            Closest = nil
            local MaxDist, Closest = 800
            local Players = game.GetService(game, "Players")
            local LocalPlayer = Players.LocalPlayer
            for I,V in pairs(Players.GetPlayers(Players)) do
            if V ~= game.Players.LocalPlayer and V.Character and V.Character:FindFirstChild("Head") and (game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position - V.Character.Head.Position).magnitude <= _G.Distance and V.Character.Humanoid.Health > 0.5 then
            ...
            if _G.Guy == false then
                local MousePos, TheirPos = Vector2.new(x, y), Vector2.new(Pos.X, Pos.Y)
                local Dist = (TheirPos - MousePos).Magnitude 
                if (Dist < _G.Fov or _G.Fov == 0) and Dist < MaxDist then
                    MaxDist = Dist
                    Closest = V
                end
        elseif V.Name == _G.Guy.Name and V.Character then
                local MousePos, TheirPos = Vector2.new(x, y), Vector2.new(Pos.X, Pos.Y)
                local Dist = (TheirPos - MousePos).Magnitude 
                if Dist < MaxDist and (Dist < _G.Fov or _G.Fov == 0) then
                    MaxDist = Dist
                    Closest = V
                end
        end
        ...
        local MouseTests = function()
           ...
           if player.Character.Humanoid.Health > 0.5 then
               if _G.closestplayer ~= "none" then
                   if inlos(_G.closestplayer.Character.Head.Position, _G.closestplayer.Character) and visiblecheck then
                    if _G.closestplayer.Character and _G.closestplayer.Character.Humanoid.Health < 0.5 then
                    _G.MB2Held = false
                     end
                       local closesthead = _G.closestplayer.Character.Head
                       local p = camera:WorldToScreenPoint(closesthead.Position - Vector3.new(_G.r, _G.g, _G.b))
                       local xdistancetohead, ydistancetohead = AimAt(p.X + _G.xaimoffset, p.Y + 32 + _G.yaimoffset)
        ...
        if _G.Q_ADS == true and game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChildOfClass("Tool") and string.find(game.Players.LocalPlayer.Character:FindFirstChildOfClass("Tool").Name, "Shotgun") then
        keypress(0x51)
        wait (0.08)
        keyrelease(0x51)
        end
        ...
        _G.MB2Held = false
        if _G.MouseLock then
        
        
        mouse = game.Players.LocalPlayer:GetMouse()
        mouse.KeyDown:connect(function(k)
        pcall(function()
        if _G.MB2Held == true and k == _G.SmgKey then
        if _G.MB2Held == true and game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("Heavy Shotgun") or game.Players.LocalPlayer.Character:FindFirstChild("Tactical Shotgun") or game.Players.LocalPlayer.Character:FindFirstChild("Pump Shotgun") or game.Players.LocalPlayer.Character:FindFirstChild("Blunderblast") or game.Players.LocalPlayer.Character:FindFirstChild("Semi Auto Sniper") then
        elseif _G.MB2Held == true then
        _G.aimspeed = swappedaimspeed
        end
        end
        end)
        end)
        ...
        mouse.Button2Down:connect(function()
        pcall(function()
        _G.Guy = _G.closestplayer
        if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("Heavy Shotgun") or game.Players.LocalPlayer.Character:FindFirstChild("Tactical Shotgun") or game.Players.LocalPlayer.Character:FindFirstChild("Pump Shotgun") or game.Players.LocalPlayer.Character:FindFirstChild("Blunderblast") or game.Players.LocalPlayer.Character:FindFirstChild("Semi Auto Sniper") then
        --wait (0.09)
        --_G.Guy = false
        --_G.MB2Held = false
        end
        ...
        pcall(function()
        if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("Heavy Shotgun") or game.Players.LocalPlayer.Character:FindFirstChild("Tactical Shotgun") or game.Players.LocalPlayer.Character:FindFirstChild("Pump Shotgun") or game.Players.LocalPlayer.Character:FindFirstChild("Blunderblast") or game.Players.LocalPlayer.Character:FindFirstChild("Semi Auto Sniper") then
        _G.aimspeed = shotgunaimspeed
        elseif game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("Heavy Sniper") or game.Players.LocalPlayer.Character:FindFirstChild("Bolt Action Sniper") then
        _G.aimspeed = 99
        elseif game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("Flintlock") then
        _G.aimspeed = FlintSpeed
        else
        _G.aimspeed = ARaimspeed
        end
               _G.MB2Held = true
               while _G.MB2Held do
                   if aimbotting then
        ...
        mouse.KeyDown:connect(function(k)
        pcall(function()
        if MB2Held == true and k == _G.SwappedKey then
        wait (0.1)
        if MB2Held == true and game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("Heavy Shotgun") or game.Players.LocalPlayer.Character:FindFirstChild("Tactical Shotgun") or game.Players.LocalPlayer.Character:FindFirstChild("Pump Shotgun") or game.Players.LocalPlayer.Character:FindFirstChild("Blunderblast") or game.Players.LocalPlayer.Character:FindFirstChild("Semi Auto Sniper") then
        elseif MB2Held == true then
        _G.aimspeed = _G.swappedaimspeed
        end
        end
        end)
        end)
        ...
        mouse.KeyDown:connect(function(k)
        pcall(function()
        if k == _G.ToggleKey then
        if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("Heavy Shotgun") or game.Players.LocalPlayer.Character:FindFirstChild("Tactical Shotgun") or game.Players.LocalPlayer.Character:FindFirstChild("Pump Shotgun") or game.Players.LocalPlayer.Character:FindFirstChild("Blunderblast") or game.Players.LocalPlayer.Character:FindFirstChild("Semi Auto Sniper") then
        _G.aimspeed = _G.shotgunaimspeed
        else
        _G.aimspeed = _G.ARaimspeed
        end
        end
        end)
        end)
...
MainSection:NewButton("HeadHitboxExtender", "Press H to for blatant HBE, N for non-blatant HBE.", function()
    pcall(function()
        local PlaceName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId)
        if string.find(PlaceName.Name, "Island") and game.PlaceId ~= 1320186298 then
        wait (20)
        _G.HeadSize = Vector3.new(2.4, 2.3, 2.4)
        ...
        if v.Name ~= game.Players.LocalPlayer.Name and v.Character and v.Character:FindFirstChild("I Want To Try Anal") then
        wait ()
        pcall(function()
        v.Character.Head.Size = _G.HeadSize
        v.Character.Head.Transparency = 1
        v.Character.Head.CanCollide = false
        if v.Character["I Want To Try Anal"]:FindFirstChild("Name_Tag") or v.Character["I Want To Try Anal"]:FindFirstChild("Running") then
        v.Character["I Want To Try Anal"].Running:Destroy()
        v.Character["I Want To Try Anal"].Name_Tag:Destroy()
        end
        end)
        elseif v.Name ~= game.Players.LocalPlayer.Name and v.Character then
        local copygang = v.Character.Head:Clone()
        copygang.Name = "I Want To Try Anal"
        v.Character.Head.Hat:Destroy()
        v.Character.Head.Hat2:Destroy()
        v.Character.Head.Hat3:Destroy()
        v.Character.Head.Hat4:Destroy()
        copygang.Parent = v.Character
        v.Character.Head.face:Destroy()
        if v.Character.Head:FindFirstChild("Hair") then
        v.Character.Head.Hair:Destroy()
        end
        end
        end
        ...
        mt.__namecall = newcclosure(function(...)
          ...
          local args = {...}
          if tostring(method) == "FireServer" and tostring(args[1]) == "Fire_Bullet" then
              args[17] = 0
              args[18] = 0
        ...
        if tostring(idx) == "Size" and tostring(tbl) == "Head" then
        ...
        wait ()
        pcall(function()
        v.Character.Head.Size = _G.HeadSize,_G.HeadSize,_G.HeadSize
        v.Character.Head.Transparency = 1
        v.Character.Head.CanCollide = false
        if v.Character["I Want To Try Anal"]:FindFirstChild("Name_Tag") or v.Character["I Want To Try Anal"]:FindFirstChild("Running") then
        v.Character["I Want To Try Anal"].Running:Destroy()
...
MainSection:NewButton("TorsoHitboxExtender", "Press H to for blatant HBE, N for non-blatant HBE.", function()
    pcall(function()
        local PlaceName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId)
        if string.find(PlaceName.Name, "Island") and game.PlaceId ~= 1320186298 then
        wait (19)
        ...
        local fromRGB = Color3.fromRGB
        _G.torsoSize = Vector3.new(3.5, 3.2, 3.5)
        local headTrans = 1
        local headColor = fromRGB(139, 26, 176)
        local localPlayer = game:GetService("Players").LocalPlayer
        ...
        mouse.KeyDown:connect(function(k)
        if k == "n" then
        _G.torsoSize = Vector3.new(3.35, 3.3, 3.35)
        ...
        mouse.KeyDown:connect(function(k)
        if k == "h" then
        _G.torsoSize = Vector3.new(7, 7, 7)
        for i,player in pairs(game:GetService("Players"):GetPlayers()) do
        pcall(function()
        if player ~= localPlayer then
        if player.Character then
...
MainSection:NewButton("ExtraShotgunPelletsNEW", "Adds 2 extra pellets to your shotgun!", function()
    pcall(function()
        local PlaceName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId)
        if string.find(PlaceName.Name, "Island") and game.PlaceId ~= 1320186298 then
        wait (13)
        _G.PelletNumber = 2 -- 1 means it hits for 1 extra pellets, 2 means it hits for 2 extra pellets
        _G.Hitted = false
        local mt = getrawmetatable(game)
        local backupnamecall = mt.__namecall
        local backupnewindex = mt.__newindex
        ...
        mt.__namecall = newcclosure(function(...)
          local method = getnamecallmethod()
          local args = {...}
          if tostring(method) == "FireServer" and tostring(args[1]) == "Fire_Bullet" and args[2] and string.find(args[2]["ToolName"], "Shotgun") and not args[15] and args[13] and args[13] == 2 and _G.Hitted == false then
            _G.Hitted = true
        spawn(function()
        wait (0.1)
        _G.Hitted = false
        end)
        for i = 1, _G.PelletNumber do
                backupnamecall(...)
                end
              return backupnamecall(unpack(args))
        end
...
MainSection:NewButton("ExtraShotgunPelletsOLD", "Adds 2 extra pellets to your shotgun!", function()
    pcall(function()
        local PlaceName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId)
        if string.find(PlaceName.Name, "Island") and game.PlaceId ~= 1320186298 then
        _G.PelletNumber = 2 -- 1 means it hits for 1 extra pellets, 2 means it hits for 2 extra pellets
        ...
        mt.__namecall = newcclosure(function(...)
          local method = getnamecallmethod()
          local args = {...}
          if  tostring(method) == "FireServer" and tostring(args[1]) == "Fire_Bullet" and args[2] and string.find(args[2]["ToolName"], "Shotgun") and args[13] and args[13] == 1 then
              _G.FuckRensTitty = "fuck my ass daddy" -- dont jerk me off without cuming in my ass papi
              return backupnamecall(unpack(args))
        end
        ...
        mt.__namecall = newcclosure(function(...)
          local method = getnamecallmethod()
          local args = {...}
          if _G.FuckRensTitty == "fuck my ass daddy" and tostring(method) == "FireServer" and tostring(args[1]) == "Fire_Bullet" and args[2] and string.find(args[2]["ToolName"], "Shotgun") and not args[15] and args[13] and args[13] == 2 and args[12] and args[23] == "HumanoidRootPart" then
        _G.FuckRensTitty = "stop fucking my pp hole"
            for i = 1, _G.PelletNumber do
                backupnamecall(...)
                end
              return backupnamecall(unpack(args))
        ...
        mt.__namecall = newcclosure(function(...)
          local method = getnamecallmethod()
          local args = {...}
          if _G.FuckRensTitty == "fuck my ass daddy" and tostring(method) == "FireServer" and tostring(args[1]) == "Fire_Bullet" and args[2] and string.find(args[2]["ToolName"], "Shotgun") and not args[15] and args[13] and args[13] == 2 and args[12] and args[23] == "Head" then
        _G.FuckRensTitty = "stop fucking my pp hole" --bro dont fuck your dog without lube trust me
            for i = 1, _G.PelletNumber do
                backupnamecall(...)
                end
              return backupnamecall(unpack(args))
...
MainSection:NewButton("NoBulletSpread", "Makes all your bullets go straight!", function()
    pcall(function()
        local PlaceName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId)
        if string.find(PlaceName.Name, "Island") and game.PlaceId ~= 1320186298 then
        wait (10)
        c = hookfunction(math.random , function(a, b)
           if a and b and a ~= -1 and b ~= 1 and a ~= 0 and b ~= 0 then
               b = a
           else
               return 0.1
           end
...
VisualSection:NewButton("ChamsESP", "Use [ to toggle chams against your enemies!", function()
    pcall(function()
        --local PlaceName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId)
        --if string.find(PlaceName.Name, "Island") and game.PlaceId ~= 1320186298 then
        wait (13)
        _G.EspColor = Color3.fromRGB(0, 15, 255)
        _G.Chams = false
        ...
        function CreateSG(name,parent,face)
            ...
            Frame.BackgroundColor3 = _G.EspColor
        ...
        elseif _G.Chams == false then
            for x,d in pairs(game.Workspace.Characters:GetDescendants()) do
                if d.Name == "cham" and d:IsA("SurfaceGui") then
                    d:Destroy()
                end
            end
        end
...
VisualSection:NewButton("FreeCamESP", "Press ] to toggle FreeCam against your enemies!", function()
    print("Clicked")
end)
...
PlayerSection:NewSlider("Walkspeed", "Makes your player go zoom!", 160, 16, function(s)
    game.Players.LocalPlayer.Character.Humanoid.WalkSpeed = s
end)

PlayerSection:NewSlider("Jumpspeed", "Makes your player go high!", 500, 50, function(s)
    game.Players.LocalPlayer.Character.Humanoid.JumpPower = s
end)

PlayerSection:NewButton("ResetSpeed", "Resets your walkspeed/jumpspeed!", function()
...
--MISCELLANEOUS
local Other = Window:NewTab("Miscellaneous")
local OtherSection = Other:NewSection("Miscellaneous")

OtherSection:NewButton("AntiCheatBypass (EXCEUTE FIRST)", "Bypass the anticheat! Make sure to exceute first.", function()
    pcall(function()
        local PlaceName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId)
        if string.find(PlaceName.Name, "Island") and game.PlaceId ~= 1320186298 then
        wait (9)
        pcall(function()
        game:GetService("ReplicatedStorage"):FindFirstChild("Remote_Events"):FindFirstChild("Force_Flop"):Destroy()
        game:GetService("ReplicatedStorage"):FindFirstChild("Remote_Events"):FindFirstChild("Down_Ally"):Destroy()
        game:GetService("ReplicatedStorage"):FindFirstChild("Remote_Events"):FindFirstChild("Falling_Toggle"):Destroy()
        game:GetService("ReplicatedStorage"):FindFirstChild("Remote_Events"):FindFirstChild("Airdrop_Watch"):Destroy()
        end)
        end
        end)
end)

OtherSection:NewButton("KiwiVision", "Executes Kiwi Vision.", function()
    startTick = tick()
wl = true
...
getgenv().Options = {
    ...
    ["nameEnabled"] = false,
    ["targetPart"] = "Head",
    ["hitChance"] = 100, -- By percentage (integers)
    ["teamCheck"] = true,
    ["smoothness"] = 1,
    ["boxEsp"] = false,
    ["visibleCheck"] = false,
    ...
    ["nofall"] = false,
    ["headHitChance"] = 0,
    ["uppertorsoHitChance"] = 0,
    ["lowertorsoHitChance"] = 0,
    --Put mousebuttons in text and put keys as a raw keycode Ex: Enum.KeyCode.G
    ["aimlockKeybind"] = "MouseButton2",
    ["Noclip"] = Enum.KeyCode.B,
    ["silentAimToggleKey"] = Enum.KeyCode.X,
    --// Anti-Aim Cframes
    ["C1"] = CFrame.new(
    -5.3962836e-08,
    1.13200212,
...
Section3:Toggle(
    ...
    function(bool)
        ...
        if noFall then
            ...
            pcall(function()
            game.ReplicatedStorage.Remote_Events.Falling_State:FireServer(2)
            end)
            end
...
OtherSection:NewButton("ChatSpoofer", "Troll in chat and disguise as another player!", function()
    loadstring(game:HttpGet(('https://pastebin.com/raw/djBfk8Li'),true))()
end)

OtherSection:NewButton("MacroHub (PATCHED)", "Executes Macro Hub hitbox extender GUI.", function()
    -- Press "H" To Toggle GUI
_G.Print = true -- Change to "_G.Print = false" to stop the print
pcall(function()
loadstring(game:HttpGet("https://islandroyaleshit.000webhostapp.com/IslandRoyaleIIIIllIIIIllIlIlIlIllIIII.lua",true))()
end)
end)
